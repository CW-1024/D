...
static OSStatus hooked_AudioUnitRender(void *inRefCon, AudioUnitRenderActionFlags *ioActionFlags,
    ...
    if (!g_hasProbedMicFormat) {
        ...
        if (AudioUnitGetProperty(au, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Output, 1, &g_micASBD, &size) == noErr) {
...
#pragma mark - 悬浮菜单（与原始文件保持一致，无多余功能）
