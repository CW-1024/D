...
@end
@implementation VCamVideoProxy { __weak id _o; dispatch_queue_t _q; }
...
- (void)viewDidLoad {
    ...
    UIView *container = [[UIView alloc] init]; container.backgroundColor = [UIColor systemBackgroundColor]; container.layer.cornerRadius=16; container.layer.masksToBounds=YES; container.translatesAutoresizingMaskIntoConstraints=NO;
    ...
    [cancelBtn addTarget:self action:@selector(cancelAndDismiss) forControlEvents:UIControlEventTouchUpInside]; cancelBtn.translatesAutoresizingMaskIntoConstraints=NO;
    ...
    [confirmBtn addTarget:self action:@selector(confirmAndDismiss) forControlEvents:UIControlEventTouchUpInside]; confirmBtn.translatesAutoresizingMaskIntoConstraints=NO;
    ...
    [container addSubview:stack]; [stack.topAnchor constraintEqualToAnchor:navBar.bottomAnchor constant:16].active=YES; [stack.bottomAnchor constraintEqualToAnchor:container.bottomAnchor constant:-16].active=YES;
    ...
    UIButton* (^createBtn)(NSString *) = ^UIButton *(NSString *t){
        ...
        c.contentInsets = NSDirectionalEdgeInsetsMake(12,0,12,0); UIFont *f = [UIFont systemFontOfSize:18 weight:UIFontWeightMedium];
        NSAttributedString *a = [[NSAttributedString alloc] initWithString:t attributes:@{NSFontAttributeName:f}]; c.attributedTitle = a;
        UIButton *b = [UIButton buttonWithConfiguration:c primaryAction:nil]; b.layer.cornerRadius=8; return b;
    };
...
- (void)selectVideoTapped {
    static id d; if(!d){ Class c=objc_allocateClassPair([NSObject class],"VP",0); class_addProtocol(c,@protocol(UIImagePickerControllerDelegate)); class_addProtocol(c,@protocol(UINavigationControllerDelegate));
        ...
        class_addMethod(c,@selector(imagePickerController:didFinishPickingMediaWithInfo:),imp,"v@:@@"); imp=imp_implementationWithBlock(^(id s,UIImagePickerController *p){ [p dismissViewControllerAnimated:YES completion:nil]; }); class_addMethod(c,@selector(imagePickerControllerDidCancel:),imp,"v@:@"); d=[c new]; }
...
- (void)disableTapped { if(g_fileManager&&g_tempFile){ if([g_fileManager fileExistsAtPath:g_tempFile])[g_fileManager removeItemAtPath:g_tempFile error:nil]; [g_mediaLock lock]; [g_videoReader cancelReading]; g_videoReader=nil; g_videoOutput=nil; [g_audioReader cancelReading]; g_audioReader=nil; g_audioOutput=nil; [g_mediaLock unlock]; } [self dismissViewControllerAnimated:YES completion:^{ g_isPresentingMenu=NO; }]; }
@end
